...
async function loadOrders() {
  ...
  const { data, error, count } = await supabase
    .from("orders")
    .select("id,customer_id,delivery_name,delivery_phone,delivery_address,subtotal,delivery_fee,discount_amount,total,payment_status,status,created_at,order_items(product_name,quantity,line_total)", { count: "exact" })
  ...
  rows.innerHTML = (data || []).map((order) => {
    ...
    return `<tr>
      <td><strong>${escapeHtml(order.id.slice(0, 8))}</strong></td>
      <td><div class="admin-order-customer"><strong>${delivery[0] || "Customer"}</strong><small>${delivery[1] || `ID ${escapeHtml(order.customer_id.slice(0, 8))}`}</small></div></td>
      <td><div class="admin-order-items">${itemMarkup}</div></td>
      <td><div class="admin-order-address">${delivery[2] || "No delivery address"}</div></td>
      <td><div class="admin-order-total"><strong>${naira.format(Number(order.total))}</strong><small>Subtotal ${naira.format(Number(order.subtotal))}</small></div></td>
