...
async function loadOrders() {
  ...
  const { data, error, count } = await supabase.from("orders").select("id,order_number,customer_id,delivery_name,delivery_phone,delivery_address,subtotal,delivery_fee,discount_amount,total,payment_status,status,created_at,order_items(product_name,quantity,line_total)", { count: "exact" }).order("created_at", { ascending: false }).range(from, to);
