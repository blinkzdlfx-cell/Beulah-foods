...
categoryForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    categoryForm.reset(); await loadCategories(); showAlert(appAlert, "Category saved.");
  } catch (error) { showAlert(appAlert, error?.message || "Could not save category.", true); }
});

productForm.addEventListener("submit", async (event) => {
  event.preventDefault(); clearAlert(appAlert);
  const submit = document.getElementById("product-submit"); submit.disabled = true;
...
productImage.addEventListener("change", () => { const file = productImage.files?.[0]; if (!file) { productPreview.hidden = true; productPreview.innerHTML = ""; return; } try { validateImage(file); renderPreview(URL.createObjectURL(file)); } catch (error) { productImage.value = ""; showAlert(appAlert, error.message, true); } });

async function loadAll() { await Promise.all([loadCategories(), loadProducts(), loadDeliverySettings(), loadPromos()]); }
...
function renderProductPagination(totalPages) {
  const root = document.getElementById("product-pagination"); root.innerHTML = "";
  if (totalPages <= 1) return;
  const previous = document.createElement("button"); previous.className = "btn btn-secondary"; previous.type = "button"; previous.textContent = "Previous"; previous.disabled = productPage <= 1; previous.onclick = () => { productPage -= 1; loadProducts(); };
  const label = document.createElement("span"); label.className = "admin-pagination__label"; label.textContent = `Page ${productPage} of ${totalPages}`;
  const next = document.createElement("button"); next.className = "btn btn-secondary"; next.type = "button"; next.textContent = "Next"; next.disabled = productPage >= totalPages; next.onclick = () => { productPage += 1; loadProducts(); };
...
function resetProductForm() { productForm.reset(); productId.value = ""; productImage.value = ""; productPreview.hidden = true; productPreview.innerHTML = ""; document.getElementById("product-submit").textContent = "Save product"; }

function validateImage(file) { if (!file) return; if (!/^image\/(jpeg|png|webp)$/.test(file.type)) throw new Error("Product images must be JPG, PNG or WebP."); if (file.size > MAX_IMAGE_SIZE) throw new Error("Product images must be 5 MB or smaller."); }
async function uploadProductImage(file) { const extension = file.type === "image/jpeg" ? "jpg" : file.type.split("/")[1]; const path = `products/${crypto.randomUUID()}.${extension}`; const { error } = await supabase.storage.from(PRODUCT_IMAGE_BUCKET).upload(path, file, { contentType: file.type, cacheControl: "31536000", upsert: false }); if (error) throw error; return path; }
async function removeProductImage(path) { if (!path || /^https?:\/\//i.test(path)) return; const { error } = await supabase.storage.from(PRODUCT_IMAGE_BUCKET).remove([path]); if (error) throw error; }
function getProductImageUrl(path) { return /^https?:\/\//i.test(path) ? path : supabase.storage.from(PRODUCT_IMAGE_BUCKET).getPublicUrl(path).data.publicUrl; }
function renderPreview(src) { productPreview.hidden = false; productPreview.innerHTML = `<img src="${escapeAttribute(src)}" alt="Product image preview">`; }
...
async function loadPromos() {
  const { data, error } = await supabase.from("promo_codes").select("id,code,discount_type,discount_value,minimum_order_amount,maximum_discount_amount,starts_at,expires_at,usage_limit,usage_count,is_active").order("created_at", { ascending: false });
  if (error) throw error;
  promoRows.innerHTML = (data || []).map((item) => `<tr><td><strong>${escapeHtml(item.code)}</strong></td><td>${item.discount_type === "percentage" ? `${item.discount_value}%` : formatNaira(item.discount_value)}</td><td>${item.usage_count}${item.usage_limit ? ` / ${item.usage_limit}` : ""}</td><td>${item.is_active ? '<span class="badge">Active</span>' : 'Inactive'}</td><td><button class="btn btn-secondary" data-promo-edit="${item.id}">Edit</button> <button class="btn btn-secondary" data-promo-toggle="${item.id}">${item.is_active ? "Deactivate" : "Activate"}</button> <button class="btn btn-secondary" data-promo-delete="${item.id}">Delete</button></td></tr>`).join("") || '<tr><td colspan="5" class="muted">No promo codes yet.</td></tr>';
  promoRows.querySelectorAll("[data-promo-edit]").forEach((button) => button.addEventListener("click", () => editPromo(data.find((item) => item.id === button.dataset.promoEdit))));
  promoRows.querySelectorAll("[data-promo-toggle]").forEach((button) => button.addEventListener("click", () => togglePromo(data.find((item) => item.id === button.dataset.promoToggle))));
  promoRows.querySelectorAll("[data-promo-delete]").forEach((button) => button.addEventListener("click", () => deletePromo(button.dataset.promoDelete)));
}
...
promoForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const starts = document.getElementById("promo-start").value; const expires = document.getElementById("promo-expires").value;
    const payload = { code: document.getElementById("promo-code").value.trim().toUpperCase(), discount_type: document.getElementById("promo-type").value, discount_value: Number(document.getElementById("promo-value").value), minimum_order_amount: Number(document.getElementById("promo-minimum").value) || 0, maximum_discount_amount: document.getElementById("promo-maximum").value ? Number(document.getElementById("promo-maximum").value) : null, usage_limit: document.getElementById("promo-limit").value ? Number.parseInt(document.getElementById("promo-limit").value, 10) : null, starts_at: starts ? new Date(starts).toISOString() : null, expires_at: expires ? new Date(expires).toISOString() : null, is_active: document.getElementById("promo-active").checked, updated_at: new Date().toISOString() };
    const query = id ? supabase.from("promo_codes").update(payload).eq("id", id) : supabase.from("promo_codes").insert(payload);
    const { error } = await query; if (error) throw error;
    resetPromoForm(); await loadPromos(); showAlert(appAlert, "Promo code saved.");
  } catch (error) { showAlert(appAlert, error.message || "Could not save promo code.", true); }
});
document.getElementById("cancel-promo").addEventListener("click", resetPromoForm);
function editPromo(item) { if (!item) return; document.getElementById("promo-id").value = item.id; document.getElementById("promo-code").value = item.code; document.getElementById("promo-type").value = item.discount_type; document.getElementById("promo-value").value = item.discount_value; document.getElementById("promo-minimum").value = item.minimum_order_amount; document.getElementById("promo-maximum").value = item.maximum_discount_amount ?? ""; document.getElementById("promo-limit").value = item.usage_limit ?? ""; document.getElementById("promo-start").value = toLocalInput(item.starts_at); document.getElementById("promo-expires").value = toLocalInput(item.expires_at); document.getElementById("promo-active").checked = item.is_active; }
function resetPromoForm() { promoForm.reset(); document.getElementById("promo-id").value = ""; }
async function togglePromo(item) { if (!item) return; const { error } = await supabase.from("promo_codes").update({ is_active: !item.is_active, updated_at: new Date().toISOString() }).eq("id", item.id); if (error) showAlert(appAlert, error.message, true); else await loadPromos(); }
async function deletePromo(id) { if (!confirm("Delete this promo code?")) return; const { error } = await supabase.from("promo_codes").delete().eq("id", id); if (error) showAlert(appAlert, error.message, true); else await loadPromos(); }
