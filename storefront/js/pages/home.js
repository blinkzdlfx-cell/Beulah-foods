...
import { getFeaturedProducts, getHomepageCategories } from "../services/catalogService.js";
...
function renderFeaturedProducts(products) {
  ...
  for (const product of products) {
    ...
    card.innerHTML = `
      <a class="product-card__media" href="${detailUrl}" aria-label="View ${escapeAttribute(product.name)}">
        ${product.image_src ? `<img class="product-card__image" src="${escapeAttribute(product.image_src)}" alt="${escapeAttribute(product.name)}" loading="lazy">` : `<span class="product-card__image-placeholder">${icon("leaf")}</span>`}
        <span class="product-card__badge">${inStock ? "In stock" : "Unavailable"}</span>
      </a>
      <div class="product-card__body">
        <p class="product-card__category">${escapeHtml(categoryName)}</p>
        <h3><a href="${detailUrl}">${escapeHtml(product.name)}</a></h3>
        <p class="product-card__meta">${escapeHtml(product.description || "Wholesome Nigerian food product.")}</p>
        <div class="product-card__footer">
          <strong>${naira.format(Number(product.price))}</strong>
          <button class="btn btn-primary product-card__add" type="button" ${inStock ? "" : "disabled"}>${inStock ? "Add to cart" : "Unavailable"}</button>
        </div>
      </div>`;
...
function renderFeaturedStructuredData(products) {
  const existing = document.getElementById("homepage-featured-schema");
  existing?.remove();
  ...
  schema.textContent = JSON.stringify({
    ...
    itemListElement: products.map((product, index) => ({
      ...
      item: {
        ...
        offers: { "@type": "Offer", priceCurrency: "NGN", price: Number(product.price), availability: Number(product.stock_quantity) > 0 ? "https://schema.org/InStock" : "https://schema.org/OutOfStock" },
      },
    })),
  });
