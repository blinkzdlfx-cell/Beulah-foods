...
async function init() {
  ...
  const [{ data: order, error: orderError }, { data: items, error: itemError }, { data: reservation, error: reservationError }] = await Promise.all([
    supabase.from("orders").select("id,status,payment_status,subtotal,delivery_fee,discount_amount,total,promo_code,created_at,delivery_name,delivery_phone,delivery_address").eq("id", id).maybeSingle(),
  ...
  if (order.status === "pending_payment" && order.payment_status === "pending" && reservation?.status === "active" && Date.parse(reservation.expires_at) > Date.now()) {
