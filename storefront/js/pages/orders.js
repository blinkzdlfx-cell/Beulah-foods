...
async function init() {
  ...
  const { data, error } = await supabase.from("orders").select("id,status,payment_status,subtotal,fees,total,created_at").order("created_at", { ascending: false });
  if (error) throw error;
  if (!data?.length) { show("You have no orders yet."); return; }
  list.innerHTML = data.map((order) => `<a class="card" style="display:block;text-decoration:none;color:inherit" href="/order.html?id=${encodeURIComponent(order.id)}"><div style="display:flex;justify-content:space-between;gap:16px;flex-wrap:wrap"><strong>Order ${order.id.slice(0,8)}</strong><span>${new Date(order.created_at).toLocaleString("en-NG")}</span></div><p style="margin:8px 0;color:var(--color-text-muted)">${order.status} · payment ${order.payment_status}</p><strong>${naira.format(Number(order.total))}</strong></a>`).join("");
}
