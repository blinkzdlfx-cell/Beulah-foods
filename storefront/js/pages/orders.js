...
async function init() {
  ...
  const { data: reservations, error: reservationError } = await supabase.from("reservations").select("id,order_id,status,expires_at").in("order_id", orderIds);
  ...
  list.innerHTML = orders.map((order) => {
    ...
    return `<article class="order-card"><a href="/order.html?id=${encodeURIComponent(order.id)}" class="order-card__main-link"><div class="order-card__top"><strong class="order-card__id">${escapeHtml(orderNumber)}</strong><span class="order-card__date">${escapeHtml(new Date(order.created_at).toLocaleString("en-NG"))}</span></div><div class="order-card__meta"><span class="order-card__status">${escapeHtml(formatStatus(order.status))}</span><span class="order-card__status">Payment: ${escapeHtml(formatStatus(order.payment_status))}</span></div><div class="order-card__total"><span>Total</span><strong>${naira.format(Number(order.total))}</strong></div></a>${activeReservation ? `<div class="order-card__reservation"><div class="order-card__reservation-copy"><span class="order-card__reservation-label">Payment reserved</span><small>Complete payment before the reservation expires.</small></div><strong class="order-card__countdown" data-expires-at="${escapeHtml(reservation.expires_at)}">--:--</strong></div><a class="order-card__retry" href="/checkout.html">Retry payment</a>` : ""}</article>`;
  }).join("");

  updateCountdowns();
  if (list.querySelector("[data-expires-at]")) countdownTimer = setInterval(updateCountdowns, 1000);
}
function updateCountdowns() { let activeCount = 0; list.querySelectorAll("[data-expires-at]").forEach((element) => { const remaining = Math.max(0, Date.parse(element.dataset.expiresAt) - Date.now()); if (remaining <= 0) { element.textContent = "Expired"; return; } activeCount += 1; const totalSeconds = Math.ceil(remaining / 1000), minutes = Math.floor(totalSeconds / 60), seconds = totalSeconds % 60; element.textContent = `${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`; }); if (!activeCount && countdownTimer) { clearInterval(countdownTimer); countdownTimer = null; } }
