...
async function findRememberedCheckoutOrder() {
  ...
  if (order.status === "cancelled") {
    return { state: "cancelled", order, reservation, items: mappedItems };
  }
...
async function init() {
  ...
  checkoutItems = sourceItems.map((item) => ({
    ...item,
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (message.includes("INSUFFICIENT_STOCK")) {
      ...
      setStatus("We could not prepare the order or payment. Please try again.", "error");
    }
  }
});
