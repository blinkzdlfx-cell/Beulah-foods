...
const list = document.getElementById("cart-list"), empty = document.getElementById("cart-empty"), summary = document.getElementById("cart-summary"), subtotalEl = document.getElementById("cart-subtotal"), selectedItemsEl = document.getElementById("cart-selected-items"), selectedCountEl = document.getElementById("cart-selected-count"), selectBar = document.getElementById("cart-select-bar"), selectAll = document.getElementById("cart-select-all"), status = document.getElementById("cart-status"), checkoutLink = document.getElementById("checkout-link");
...
function getSelection() {
  try { const value = JSON.parse(sessionStorage.getItem(SELECTION_KEY) || "[]"); return new Set(Array.isArray(value) ? value.map(String) : []); } catch { return new Set(); }
}
function saveSelection(selection) { sessionStorage.setItem(SELECTION_KEY, JSON.stringify([...selection])); }
...
function setQuantity(productId, quantity, max) { const nextQuantity = Math.max(1, Math.min(max, Number(quantity) || 1)); updateCartQuantity(productId, nextQuantity); render(); showToast(`Quantity updated to ${nextQuantity}.`); }
...
function updateSelectionUI(validItems) {
  ...
  const selectedQuantity = selectedItems.reduce((total, item) => total + Math.min(item.quantity, Math.max(1, Number(item.product.stock_quantity) || 1)), 0);
  const selectedSubtotal = selectedItems.reduce((total, item) => total + Number(item.product.price) * Math.min(item.quantity, Math.max(1, Number(item.product.stock_quantity) || 1)), 0);
  ...
  checkoutLink.href = selectedItems.length ? `checkout.html?items=${encodeURIComponent(selectedItems.map((item) => item.product.id).join(","))}` : "#";
}
...
async function render() {
  ...
  try {
    ...
    for (const item of validItems) {
      ...
      checkbox.addEventListener("change", () => { const next = getSelection(); if (checkbox.checked) next.add(String(product.id)); else next.delete(String(product.id)); saveSelection(next); render(); });
      ...
      row.querySelector(".cart-remove").addEventListener("click", () => { const next = getSelection(); next.delete(String(product.id)); saveSelection(next); removeFromCart(product.id); render(); showToast(`${product.name} removed from cart.`); });
      list.append(row);
    }
