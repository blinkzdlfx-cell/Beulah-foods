...
async function render() {
  ...
  try {
    const cart = getCart(), products = await getProducts(), productMap = new Map(products.map((product) => [String(product.id), product]));
    ...
    const hasItems = validItems.length > 0; empty.hidden = hasItems; summary.hidden = !hasItems; subtotalEl.textContent = naira.format(subtotal); checkoutLink.hidden = !hasItems; setStatus("");
  } catch (error) { console.error(error); setStatus("We could not load your cart. Please try again.", "error"); }
}
render();
