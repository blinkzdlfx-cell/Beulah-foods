-- Phase 2 refinement: delivery is optional and its two behaviours are independently controlled.
-- No active delivery setting means checkout proceeds with no delivery charge.

alter table public.delivery_settings
  add column if not exists is_delivery_enabled boolean not null default false,
  add column if not exists is_free_delivery_enabled boolean not null default false;

alter table public.delivery_settings
  alter column delivery_fee drop not null,
  alter column free_delivery_threshold drop not null;

alter table public.delivery_settings
  drop constraint if exists delivery_settings_delivery_fee_check,
  drop constraint if exists delivery_settings_free_delivery_threshold_check;

alter table public.delivery_settings
  add constraint delivery_settings_delivery_fee_check
    check (delivery_fee is null or delivery_fee >= 0),
  add constraint delivery_settings_free_delivery_threshold_check
    check (free_delivery_threshold is null or free_delivery_threshold >= 0),
  add constraint delivery_settings_delivery_config_valid
    check (not is_delivery_enabled or delivery_fee is not null),
  add constraint delivery_settings_free_delivery_config_valid
    check (not is_free_delivery_enabled or (is_delivery_enabled and free_delivery_threshold is not null));

-- Preserve any already-configured active delivery rule from the previous model.
update public.delivery_settings
set is_delivery_enabled = true,
...
comment on table public.delivery_settings is 'Optional admin-controlled delivery settings. Delivery and free-delivery behaviour can be enabled independently.';
