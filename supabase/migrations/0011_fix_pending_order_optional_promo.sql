-- Fix: create_pending_order must work when no promo code is supplied.
-- PostgreSQL RECORD variables are unassigned when SELECT INTO finds no row,
-- so do not read promo_row fields unless a promo was actually found.
...
begin
  ...
  set subtotal = v_subtotal,
      ...
      promo_code = case when v_promo_applied then promo_row.code else null end,
      promo_discount_type = case when v_promo_applied then promo_row.discount_type else null end,
      promo_discount_value = case when v_promo_applied then promo_row.discount_value else null end,
      promo_minimum_order = case when v_promo_applied then promo_row.minimum_order_amount else null end,
      promo_maximum_discount = case when v_promo_applied then promo_row.maximum_discount_amount else null end,
      total = v_total,
      updated_at = now()
  where id = order_id;
