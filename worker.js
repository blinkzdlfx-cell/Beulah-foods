...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname.endsWith(".html") && !url.pathname.startsWith("/storefront/") && !url.pathname.startsWith("/admin/")) {
      const storefrontUrl = new URL(request.url);
...
async function handlePaystackInitialize(request, env, url) {
  ...
  const orderResult = await supabaseRest(env, `/rest/v1/orders?id=eq.${encodeURIComponent(orderId)}&customer_id=eq.${encodeURIComponent(user.id)}&select=id,customer_id,total,payment_status,status`);
...
async function handlePaystackVerify(request, env, url) {
  ...
  const paymentResult = await supabaseRest(env, `/rest/v1/payments?provider_reference=eq.${encodeURIComponent(reference)}&provider=eq.paystack&select=id,order_id,amount,status`);
  const payment = paymentResult.data?.[0];
  if (!payment) return json({ error: "PAYMENT_NOT_FOUND" }, 404);
  const orderResult = await supabaseRest(env, `/rest/v1/orders?id=eq.${encodeURIComponent(payment.order_id)}&customer_id=eq.${encodeURIComponent(user.id)}&select=id,total,status,payment_status`);
  const order = orderResult.data?.[0];
  if (!order) return json({ error: "ORDER_NOT_FOUND" }, 404);
...
async function sendOrderEmails(env, orderId, reference) {
  ...
  const orderResult = await supabaseRest(env, `/rest/v1/orders?id=eq.${encodeURIComponent(orderId)}&select=id,total,subtotal,fees,delivery_fee,discount_amount,promo_code,delivery_name,delivery_address,created_at`);
  const order = orderResult.data?.[0];
  if (!order) return;
  const userIdResult = await supabaseRest(env, `/rest/v1/orders?id=eq.${encodeURIComponent(orderId)}&select=customer_id`);
  const customerId = userIdResult.data?.[0]?.customer_id;
  if (!customerId) return;
  const userResponse = await fetch(`${env.SUPABASE_URL}/auth/v1/admin/users/${encodeURIComponent(customerId)}`, { headers: { apikey: env.SUPABASE_SERVICE_ROLE_KEY, Authorization: `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}` } });
  const authUser = await safeJson(userResponse);
  const customerEmail = authUser?.email;
  if (!customerEmail) return;

  const itemsResult = await supabaseRest(env, `/rest/v1/order_items?order_id=eq.${encodeURIComponent(orderId)}&select=product_name,quantity,line_total&order=id.asc`);
  const itemsHtml = (itemsResult.data || []).map((item) => `<li>${escapeHtml(item.product_name)} × ${item.quantity} — ${formatNaira(item.line_total)}</li>`).join("");
  ...
  await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${env.RESEND_API_KEY}`, "Content-Type": "application/json", "Idempotency-Key": `order-confirmation/${reference}` }, body: JSON.stringify({ from: env.RESEND_FROM, to: [customerEmail], subject: `Beulah Foods order ${order.id.slice(0, 8)} confirmed`, html }) });
  if (env.ORDER_NOTIFICATION_EMAIL) {
    await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${env.RESEND_API_KEY}`, "Content-Type": "application/json", "Idempotency-Key": `admin-order/${reference}` }, body: JSON.stringify({ from: env.RESEND_FROM, to: [env.ORDER_NOTIFICATION_EMAIL], subject: `New Beulah Foods order ${order.id.slice(0, 8)}`, html }) });
  }
}

async function getSupabaseUser(env, accessToken) {
  const response = await fetch(`${env.SUPABASE_URL}/auth/v1/user`, { headers: { apikey: env.SUPABASE_SERVICE_ROLE_KEY, Authorization: `Bearer ${accessToken}` } });
...
async function supabaseRpc(env, functionName, body) {
  ...
  const response = await fetch(`${env.SUPABASE_URL}/rest/v1/rpc/${functionName}`, { method: "POST", headers, body: JSON.stringify(body) });
  return { ok: response.ok, status: response.status, data: await safeJson(response) };
}
